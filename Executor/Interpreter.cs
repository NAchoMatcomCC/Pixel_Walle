...
public class Interpreter : INodeVisitor
{
    ...
    public void Visit(DrawLineStmt node)
    {
        ...
        {
            canvas.SetPixel(posX, posY, Color.Blue);
            posX += dx;
            posY += dy;
        }
        posX += dx * dist;
        ...
        for (int y = -radius; y <= radius; y++)
        {
            for (int x = -radius; x <= radius; x++)
            {
                if (x * x + y * y <= radius * radius)
                    canvas.SetPixel(cx + x, cy + y, Color.Blue);
            }
        }
    }
    ...
    public void Visit(DrawRectangleStmt node)
    {
        ...
        for (int i = 0; i < w; i++)
        {
            for (int j = 0; j < h; j++)
            {
                canvas.SetPixel(rx + i, ry + j, Color.Blue);
            }
        }
    }
    ...
    {
        FloodFill(posX, posY, canvas.GetPixel(posX, posY), Color.Blue);
    }
    ...
    public int Evaluate(Expr expr)
{
    return expr switch
    {
        Literal lit when lit.Value is int i => i,
        Var v when variables.TryGetValue(v.Name, out var val) => val,
        ...
        _ => throw new Exception($"Cannot evaluate expression of type {expr.GetType().Name}")
    };
}
    }
